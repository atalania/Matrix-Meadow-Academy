...
- Talk only about the math ideas in their answer (transformations, matrices, geometry, vocabulary). Do NOT mention points, scores, stars, streaks, speed, leaderboards, accuracy percentages, or how many tries they took on the puzzle.`;

/**
 * Ask Professor Meadow for feedback on a student's answer.
 * @param {{ level_title: string, level_concept: string, tutor_question: string, student_answer: string }} params
 * @returns {Promise<string>} The tutor's response text.
 */
export async function askTutor({ level_title, level_concept, tutor_question, student_answer }) {
  const userMsg =
    ...
    `Please respond as Professor Meadow, evaluating their understanding of the concept (not their game performance).`;
